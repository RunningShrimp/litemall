<template>
    <uni-shadow-root class="vant-skeleton-index">
        <view :class="'custom-class '+(utils.bem('skeleton', [{animate}]))" v-if="loading">
            <view :class="'avatar-class '+(utils.bem('skeleton__avatar', [avatarShape]))"
                  :style="'width:' + avatarSize + ';height:' + avatarSize" v-if="avatar"></view>
            <view :class="utils.bem('skeleton__content')">
                <view :class="'title-class '+(utils.bem('skeleton__title'))" :style="'width:' + titleWidth"
                      v-if="title"></view>
                <view :class="'row-class '+(utils.bem('skeleton__row'))" :key="item.index"
                      :style="'width:' + (isArray ? rowWidth[index] : rowWidth)" v-for="(item,index) in (row)"></view>
            </view>
        </view>
