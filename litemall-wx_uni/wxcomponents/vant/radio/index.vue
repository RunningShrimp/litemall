...
<uni-shadow-root class="vant-radio-index"><view class="van-radio custom-class">
  <view :class="'label-class '+(utils.bem('radio__label', [labelPosition, { disabled }]))" @click="onClickLabel" v-if="labelPosition === 'left'">
    <slot></slot>
  </view>
  <view :style="'font-size: '+(utils.addUnit(iconSize))+';'" @click="onChange" class="van-radio__icon-wrap">
    <slot name="icon" v-if="useIconSlot"></slot>
    <van-icon :class="utils.bem('radio__icon', [shape, { disabled, checked: value === name }])" :custom-style="'line-height: '+(utils.addUnit(iconSize))+';font-size: .8em;display: block;'" :style="'font-size: '+(utils.addUnit(iconSize))+';'+(checkedColor && !disabled && value === name ? 'border-color:' + checkedColor + '; background-color:' + checkedColor + ';' : '')" custom-class="icon-class" name="success" v-else></van-icon>
  </view>
  <view :class="'label-class '+(utils.bem('radio__label', [labelPosition, { disabled }]))" @click="onClickLabel" v-if="labelPosition === 'right'">
    <slot></slot>
  </view>
