    ...
    <textarea :adjust-position="adjustPosition" :auto-height="autosize"
              ...
              v-if="type === 'textarea'">
                ...
                <input :adjust-position="adjustPosition"
                       ...
                       :disabled="disabled || readonly" :focus="focus" :hold-keyboard="holdKeyboard"
                       :maxlength="maxlength" :password="password || type === 'password'" :placeholder="placeholder"
                       :placeholder-class="utils.bem('field__placeholder', { error })"
                       :placeholder-style="placeholderStyle" :selection-end="selectionEnd"
                       :selection-start="selectionStart" :type="type" :value="value" @blur="onBlur" @confirm="onConfirm"
                       @focus="onFocus" @input="onInput" v-else></input>
                <van-icon @touchstart.native.stop.prevent="onClear" class="van-field__clear-root van-field__icon-root"
                          name="clear" size="16px" v-if="clearable && focused && value && !readonly"></van-icon>
