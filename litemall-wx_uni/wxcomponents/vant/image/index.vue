<template>
    <uni-shadow-root class="vant-image-index">
        <view :class="'custom-class '+(utils.bem('image', { round }))" :style="style" @click="onClick">
            <image :lazy-load="lazyLoad" :mode="mode" :show-menu-by-longpress="showMenuByLongpress" :src="src"
                   @error="onImageError" @load="onImageLoad" class="image-class van-image__img" v-if="(!error)"></image>
