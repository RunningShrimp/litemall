<template>
    <uni-shadow-root class="vant-toast-index">
        <van-overlay :custom-style="mask ? '' : 'background-color: transparent;'" :show="show" :z-index="zIndex"
                     v-if="mask || forbidClick"></van-overlay>
        <van-transition :custom-style="'z-index: '+(zIndex)" :show="show" custom-class="van-toast__container">
            <view :class="'van-toast van-toast--'+(type === 'text' ? 'text' : 'icon')+' van-toast--'+(position)"
                ...
                <block v-else>
                    <van-loading :type="loadingType" color="white" custom-class="van-toast__loading"
                                 v-if="type === 'loading'"></van-loading>
