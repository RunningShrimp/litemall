...
<uni-shadow-root class="vant-submit-bar-index"><view class="van-submit-bar custom-class">
  ...
  <view class="van-submit-bar__tip">
    ...
    <view class="van-submit-bar__tip-text" v-if="hasTip">
      {{ tip }}
    </view>
    <slot name="tip"></slot>
  </view>
  ...
  <view :class="'bar-class '+(utils.bem('submit-bar__bar', { safe: safeAreaInsetBottom }))">
    ...
    <van-button :disabled="disabled" :loading="loading" :type="buttonType" @click="onSubmit" class="van-submit-bar__button" custom-class="button-class" size="large" square>
      {{ loading ? '' : buttonText }}
    </van-button>
  </view>
