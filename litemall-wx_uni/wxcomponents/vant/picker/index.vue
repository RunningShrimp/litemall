...
<uni-shadow-root class="vant-picker-index"><template name="toolbar">
  <view class="van-picker__toolbar van-hairline--top-bottom toolbar-class" v-if="showToolbar">
    <view @click="emit" class="van-picker__cancel" data-type="cancel" hover-class="van-picker__cancel--hover" hover-stay-time="70">
      {{ cancelButtonText }}
    </view>
    <view class="van-picker__title van-ellipsis" v-if="title">{{ title }}</view>
    <view @click="emit" class="van-picker__confirm" data-type="confirm" hover-class="van-picker__confirm--hover" hover-stay-time="70">
      {{ confirmButtonText }}
    </view>
  </view>
</template>
...
<view class="van-picker custom-class">
  <template :data="showToolbar, cancelButtonText, title, confirmButtonText" is="toolbar" v-if="toolbarPosition === 'top'"></template>
  <view class="van-picker__loading" v-if="loading">
    <loading color="#1989fa"></loading>
  </view>
  <view :style="'height: '+(itemHeight * visibleItemCount)+'px'" @touchmove.stop.prevent="noop" class="van-picker__columns">
    <picker-column :data-index="index" :default-index="item.defaultIndex || defaultIndex" :initial-options="isSimple(columns) ? item : item.values" :item-height="itemHeight" :key="item.index" :value-key="valueKey" :visible-item-count="visibleItemCount" @change="onChange" active-class="active-class" class="van-picker__column" custom-class="column-class" v-for="(item,index) in (isSimple(columns) ? [columns] : columns)"></picker-column>
    <view :style="'height: '+(itemHeight)+'px'" class="van-picker__frame van-hairline--top-bottom"></view>
  </view>
  <template :data="showToolbar, cancelButtonText, title, confirmButtonText" is="toolbar" v-if="toolbarPosition === 'bottom'"></template>
