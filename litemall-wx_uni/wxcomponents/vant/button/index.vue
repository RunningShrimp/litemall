<template>
<uni-shadow-root class="vant-button-index"><button :app-parameter="appParameter" :aria-label="ariaLabel" :business-id="businessId" :class="'custom-class '+(utils.bem('button', [type, size, { block, round, plain, square, loading, disabled, hairline, unclickable: disabled || loading }]))+' '+(hairline ? 'van-hairline--surround' : '')" :id="id" :lang="lang" :open-type="openType" :send-message-img="sendMessageImg" :send-message-path="sendMessagePath" :send-message-title="sendMessageTitle" :session-from="sessionFrom" :show-message-card="showMessageCard" :style="(style)+' '+(customStyle)" @click="onClick" @contact="bindContact" @error="bindError" @getphonenumber="bindGetPhoneNumber" @getuserinfo="bindGetUserInfo" @launchapp="bindLaunchApp" @opensetting="bindOpenSetting" hover-class="van-button--active hover-class">
  <block v-if="loading">
    <van-loading :color="type === 'default' ? '#c9c9c9' : 'white'" :size="loadingSize" :type="loadingType" custom-class="loading-class"></van-loading>
    <view class="van-button__loading-text" v-if="loadingText">
      {{ loadingText }}
    </view>
  </block>
  <block v-else>
    <van-icon :name="icon" class="van-button__icon" custom-style="line-height: inherit;" size="1.2em" v-if="icon"></van-icon>
