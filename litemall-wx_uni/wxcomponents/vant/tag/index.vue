<template>
    <uni-shadow-root class="vant-tag-index">
        <view :class="'custom-class '+(utils.bem('tag', [type, size, { mark, plain, round }]))+' '+(plain ? 'van-hairline--surround' : '')"
              :style="(color && !plain ? 'background-color: ' + color + ';' : '')+(textColor || (color && plain) ? 'color: ' + (textColor || color) : '')">
            <slot></slot>
            <van-icon @click="onClose" custom-class="van-tag__close" name="cross" v-if="closeable"></van-icon>
        </view>
    </uni-shadow-root>
</template>
...
<style platform="mp-weixin">
    ...
    .van-tag {
        ...
        line-height: normal;
        padding: .2em .5em;
        padding: var(--tag-padding, .2em .5em);
        color: #fff;
        color: var(--tag-text-color, #fff);
        font-size: 10px;
        font-size: var(--tag-font-size, 10px);
        border-radius: .2em;
        border-radius: var(--tag-border-radius, .2em)
    }

    .van-tag:after {
        border-color: currentColor;
        border-radius: .2em *2;
        border-radius: var(--tag-border-radius, .2em) *2
    }
