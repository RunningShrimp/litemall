<template>
    <uni-shadow-root class="vant-goods-action-button-index">
        <van-button :app-parameter="appParameter" :business-id="businessId" :color="color"
                    ...
                    :disabled="disabled" :id="id" :lang="lang" :loading="loading" :open-type="openType" :plain="plain"
                    :send-message-img="sendMessageImg" :send-message-path="sendMessagePath"
                    :send-message-title="sendMessageTitle" :session-from="sessionFrom"
                    :show-message-card="showMessageCard" :type="type" @click="onClick" @contact="bindContact"
                    @error="bindError" @getphonenumber="bindGetPhoneNumber" @getuserinfo="bindGetUserInfo"
                    @launchapp="bindLaunchApp" @opensetting="bindOpenSetting" square>
            {{ text }}
        </van-button>
    </uni-shadow-root>
</template>
...
<style platform="mp-weixin">
    ...
    .van-goods-action-button--ordinary {
        border: none !important
    }

    .van-goods-action-button--plain {
        background: #fff;
        background: var(--goods-action-button-plain-color, #fff)
    }

    .van-goods-action-button--no-right-border {
