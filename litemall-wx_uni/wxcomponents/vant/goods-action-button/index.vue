<template>
<uni-shadow-root class="vant-goods-action-button-index"><van-button :app-parameter="appParameter" :business-id="businessId" :color="color" :custom-class="(utils.bem('goods-action-button', [type, { first: isFirst, last: isLast, plain : plain, ordinary: !plain }]))+' '+(rightBorderLess ?'van-goods-action-button--no-right-border': '')" :disabled="disabled" :id="id" :lang="lang" :loading="loading" :open-type="openType" :plain="plain" :send-message-img="sendMessageImg" :send-message-path="sendMessagePath" :send-message-title="sendMessageTitle" :session-from="sessionFrom" :show-message-card="showMessageCard" :type="type" @click="onClick" @contact="bindContact" @error="bindError" @getphonenumber="bindGetPhoneNumber" @getuserinfo="bindGetUserInfo" @launchapp="bindLaunchApp" @opensetting="bindOpenSetting" square>
  {{ text }}
