...
<uni-shadow-root class="vant-uploader-index"><view class="van-uploader">
  <view class="van-uploader__wrapper">
    ...
    <view :key="item.index" class="van-uploader__preview" v-for="(item,index) in (lists)" v-if="previewImage">
      <image :alt="item.name || ('图片' + index)" :data-url="item.url || item.path" :mode="imageFit" :src="item.url || item.path" :style="'width: '+(utils.addUnit(previewSize))+'; height: '+(utils.addUnit(previewSize))+';'" @click="doPreviewImage" class="van-uploader__preview-image" v-if="item.isImage"></image>
      <view :style="'width: '+(utils.addUnit(previewSize))+'; height: '+(utils.addUnit(previewSize))+';'" class="van-uploader__file" v-else>
      ...
      <van-icon :data-index="index" @click.native="deleteItem" class="van-uploader__preview-delete" name="clear" v-if="deletable"></van-icon>
    </view>

    
    <block v-if="isInCount">
      <view @click="startUpload" class="van-uploader__slot">
        <slot></slot>
      </view>
