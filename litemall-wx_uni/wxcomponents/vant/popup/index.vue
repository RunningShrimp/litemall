<template>
<uni-shadow-root class="vant-popup-index"><van-overlay :custom-style="overlayStyle" :duration="duration" :show="show" :z-index="zIndex" @click="onClickOverlay" v-if="overlay"></van-overlay>
<view :class="'custom-class '+(classes)+' '+(utils.bem('popup', [position, { round, safe: safeAreaInsetBottom, safeTop: safeAreaInsetTop }]))" :style="'z-index: '+(zIndex)+'; -webkit-transition-duration:'+(currentDuration)+'ms; transition-duration:'+(currentDuration)+'ms; '+(display ? '' : 'display: none;')+';'+(customStyle)" @transitionend="onTransitionEnd" v-if="inited">
  <slot></slot>
  <van-icon :class="'van-popup__close-icon van-popup__close-icon--'+(closeIconPosition)" :name="closeIcon" @click.native="onClickCloseIcon" v-if="closeable"></van-icon>
