<template>
    <uni-shadow-root class="vant-tree-select-index">
        <view :style="'height: '+(utils.addUnit(height))" class="van-tree-select">
            <scroll-view class="van-tree-select__nav" scroll-y>
                <van-sidebar :active-key="mainActiveIndex" @change="onClickNav"
                             custom-class="van-tree-select__nav__inner">
                    <van-sidebar-item :disabled="item.disabled" :key="item.index" :title="item.text"
            ...
            <scroll-view class="van-tree-select__content" scroll-y>
                ...
                <view :class="'van-ellipsis content-item-class '+(utils.bem('tree-select__item', { active: wxs.isActive(activeId, item.id), disabled: item.disabled }))+' '+(wxs.isActive(activeId, item.id) ? 'content-active-class' : '')+' '+(item.disabled ? 'content-disabled-class' : '')"
                      :data-item="item" :key="item.id" @click="onSelectItem" v-for="(item,index) in (subItems)">
                    {{ item.text }}
...
<script>
    ...
    global['__wxVueOptions'] = {
        components: {
            ...
            'van-sidebar-item': VanSidebarItem
        }
    }
