        ...
        <van-popup :close-on-click-overlay="closeOnClickOverlay" :overlay="overlay" :round="round"
            ...
            <view v-if="actions && actions.length">
                ...
                <button :app-parameter="appParameter"
                        ...
                        :show-message-card="showMessageCard" :style="item.color ? 'color: ' + item.color : ''"
                        @click="onSelect" @contact="bindContact" @error="bindError" @getphonenumber="bindGetPhoneNumber"
            ...
            <slot></slot>
            <view @click="onCancel" class="van-action-sheet__cancel" hover-class="van-action-sheet__cancel--hover"
                  hover-stay-time="70" v-if="cancelText">
                {{ cancelText }}
            </view>
        </van-popup>
    </uni-shadow-root>
