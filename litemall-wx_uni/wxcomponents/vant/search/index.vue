...
<uni-shadow-root class="vant-search-index"><view :class="(utils.bem('search', { withaction: showAction || useActionSlot }))+' custom-class'" :style="'background: '+(background)">
  <view :class="utils.bem('search__content', [shape])">
    ...
    <van-field :border="false" :clearable="clearable" :disabled="disabled" :error="error" :focus="focus" :input-align="inputAlign" :left-icon="(!useLeftIconSlot ? leftIcon : '')" :maxlength="maxlength" :placeholder="placeholder" :placeholder-style="placeholderStyle" :readonly="readonly" :right-icon="(!useRightIconSlot ? rightIcon : '')" :value="value" @blur="onBlur" @change="onChange" @clear="onClear" @confirm="onSearch" @focus="onFocus" class="van-search__field field-class" confirm-type="search" custom-style="padding: 5px 10px 5px 0; background-color: transparent;" input-class="input-class" type="search">
