<template>
<uni-shadow-root class="vant-picker-column-index"><view :style="'height: '+(itemHeight * visibleItemCount)+'px'" @touchcancel="onTouchEnd" @touchend="onTouchEnd" @touchmove.stop.prevent="onTouchMove" @touchstart="onTouchStart" class="van-picker-column custom-class">
  <view :style="'transition: transform '+(duration)+'ms; line-height: '+(itemHeight)+'px; transform: translate3d(0, '+(offset + (itemHeight * (visibleItemCount - 1)) / 2)+'px, 0)'">
    <view :class="'van-ellipsis van-picker-column__item '+(option && option.disabled ? 'van-picker-column__item--disabled' : '')+' '+(index === currentIndex ? 'van-picker-column__item--selected active-class' : '')" :data-index="index" :key="option.index" :style="'height: '+(itemHeight)+'px'" @click="onClickItem" v-for="(option,index) in (options)">{{ getOptionText(option, valueKey) }}</view>
  </view>
