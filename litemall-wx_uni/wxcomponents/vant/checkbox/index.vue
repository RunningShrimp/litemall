...
<uni-shadow-root class="vant-checkbox-index"><view class="van-checkbox custom-class">
  <view @click="toggle" class="van-checkbox__icon-wrap">
    <slot name="icon" v-if="useIconSlot"></slot>
    <van-icon :class="utils.bem('checkbox__icon', [shape, { disabled, checked: value }])" :style="'font-size: '+(utils.addUnit(iconSize))+';'+(checkedColor && value && !disabled ? 'border-color:' + checkedColor + '; background-color:' + checkedColor : '')" custom-class="icon-class" custom-style="line-height: 1.25em;" name="success" size="0.8em" v-else></van-icon>
  </view>
