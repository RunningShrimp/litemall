...
<uni-shadow-root class="vant-steps-index"><view :class="'custom-class '+(utils.bem('steps', [direction]))">
  <view class="van-step__wrapper">
    <view :class="(utils.bem('step', [direction, status(index, active)]))+' van-hairline'" :key="item.index" :style="status(index, active) === 'inactive' ? 'color: ' + inactiveColor: ''" v-for="(item,index) in (steps)">
      ...
      <view class="van-step__circle-container">
        <block v-if="index !== active">
          <van-icon :color="status(index, active) === 'inactive' ? inactiveColor: activeColor" :name="inactiveIcon" custom-class="van-step__icon" v-if="inactiveIcon"></van-icon>
          <view :style="'background-color: ' + (index < active ? activeColor : inactiveColor)" class="van-step__circle" v-else></view>
        </block>

        <van-icon :color="activeColor" :name="activeIcon" custom-class="van-step__icon" v-else></van-icon>
      </view>
      <view :style="'background-color: ' + (index < active ? activeColor : inactiveColor)" class="van-step__line" v-if="index !== steps.length - 1"></view>
    </view>
  </view>
</view></uni-shadow-root>
</template>
<wxs module="utils" src="../wxs/utils.wxs"></wxs><wxs module="status" src="./index-status.wxs"></wxs>
<script>
