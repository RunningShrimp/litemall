<template>
    <uni-shadow-root class="vant-index-bar-index">
        <view class="van-index-bar">
            ...
            <view @click.stop.prevent="onClick" @touchcancel.stop.prevent="onTouchStop"
                  @touchend.stop.prevent="onTouchStop" @touchmove.stop.prevent="onTouchMove"
                  class="van-index-bar__sidebar" v-if="showSidebar">
                <view :data-index="index" :key="item.index"
                      :style="'z-index: '+(zIndex + 1)+'; color: '+(activeAnchorIndex === index ? highlightColor : '')"
                      class="van-index-bar__index" v-for="(item,index) in (indexList)">
                    {{ item }}
                </view>
            </view>
        </view>
