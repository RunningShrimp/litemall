<template>
<uni-shadow-root class="vant-notify-index"><van-transition :custom-style="'z-index: '+(zIndex)+';'" :show="show" @click.native="onTap" custom-class="van-notify__container" name="slide-down">
  <view :class="'van-notify van-notify--'+(type)" :style="'background:'+(background)+';color:'+(color)+';'">
    <view :style="'height: '+(statusBarHeight)+'px'" v-if="safeAreaInsetTop"></view>
    <text>{{ message }}</text>
  </view>
