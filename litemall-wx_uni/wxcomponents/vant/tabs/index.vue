    ...
    <view :class="(utils.bem('tabs__wrap', { scrollable }))+' '+(type === 'line' && border ? 'van-hairline--top-bottom' : '')">
      ...
      <scroll-view :class="utils.bem('tabs__scroll', [type])" :scroll-left="scrollLeft" :scroll-x="scrollable" :style="color ? 'border-color: ' + color : ''" scroll-with-animation>
        <view :class="(utils.bem('tabs__nav', [type]))+' nav-class'">
          ...
          <view :class="(getters.tabClass(index === currentIndex, ellipsis))+' '+(utils.bem('tab', { active: index === currentIndex, disabled: item.disabled, complete: !ellipsis }))" :data-index="index" :key="item.index" :style="getters.tabStyle(index === currentIndex, ellipsis, color, type, item.disabled, titleActiveColor, titleInactiveColor, swipeThreshold, scrollable)" @click="onTap" v-for="(item,index) in (tabs)">
            <view :class="ellipsis ? 'van-ellipsis' : ''" :style="item.titleStyle">
              {{ item.title }}
              <van-info :dot="item.dot" :info="item.info" custom-class="van-tab__title__info" v-if="item.info !== null || item.dot"></van-info>
            </view>
          </view>
        </view>
      </scroll-view>
...
<wxs module="utils" src="../wxs/utils.wxs"></wxs><wxs module="getters" src="./index.wxs"></wxs>
<script>
