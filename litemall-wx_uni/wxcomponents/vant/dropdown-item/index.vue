    ...
    <uni-shadow-root class="vant-dropdown-item-index">
        <view :class="utils.bem('dropdown-item', direction)" :style="wrapperStyle" v-if="showWrapper">
            <van-popup :close-on-click-overlay="closeOnClickOverlay" :duration="transition ? duration : 0"
                ...
                <van-cell :class="utils.bem('dropdown-item__option', { active: item.value === value } )"
                          :data-option="item" :icon="item.icon" :key="item.value" @click.native="onOptionTap" clickable
