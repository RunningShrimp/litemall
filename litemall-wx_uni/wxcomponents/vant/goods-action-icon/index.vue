<template>
<uni-shadow-root class="vant-goods-action-icon-index"><van-button :app-parameter="appParameter" :business-id="businessId" :disabled="disabled" :id="id" :lang="lang" :loading="loading" :open-type="openType" :send-message-img="sendMessageImg" :send-message-path="sendMessagePath" :send-message-title="sendMessageTitle" :session-from="sessionFrom" :show-message-card="showMessageCard" @click="onClick" @contact="bindContact" @error="bindError" @getphonenumber="bindGetPhoneNumber" @getuserinfo="bindGetUserInfo" @launchapp="bindLaunchApp" @opensetting="bindOpenSetting" custom-class="van-goods-action-icon" size="large" square>
  <view class="van-goods-action-icon__content">
    <van-icon :dot="dot" :info="info" :name="icon" class="van-goods-action-icon__icon" custom-class="icon-class" size="20px" v-if="icon"></van-icon>
    <slot name="icon"></slot>
