...
<scroll-view class="container" style="height: 100%;">
  <view class="search-header">
    <view class="input-box">
      ...
      <input :placeholder="defaultKeyword.keyword" :value="keyword" @confirm="onKeywordConfirm" @focus="inputFocus" @input="inputChange" class="keywrod" confirm-type="search" focus="true" name="input"></input>
  ...
  <view class="no-search" v-if="!searchStatus">
    <view class="search-keywords search-history" v-if="!keyword  && historyKeyword.length">
      ...
      <view class="b">
        <view :data-keyword="item.keyword" :key="keyword" @tap="onKeywordTap" class="item" hover-class="navigator-hover" v-for="(item, keyword) in historyKeyword">{{item.keyword}}</view>
      </view>
    </view>
    <view class="search-keywords search-hot" v-if="!keyword && hotKeyword.length">
      ...
      <view class="b">
        <view :class="'item ' + (item.is_hot === 1 ? 'active' : '')" :data-keyword="item.keyword" :key="id" @tap="onKeywordTap" hover-class="navigator-hover" v-for="(item, id) in hotKeyword">{{item.keyword}}</view>
      </view>
    </view>
    <view class="shelper-list" v-if="keyword">
      <view :data-keyword="item" :key="id" @tap="onKeywordTap" class="item" hover-class="navigator-hover" v-for="(item, id) in helpKeyword">{{item}}</view>
    </view>
  </view>
    ...
    <view class="sort">
      <view class="sort-box">
        ...
        <view :class="'item ' + (currentSortType == 'price' ? 'active' : '')" @tap="openSortFilter" id="priceSort">
          ...
          <van-icon name="arrow-down" v-else-if="currentSortType == 'price' && currentSortOrder == 'desc'"></van-icon>
        </view>
      ...
      <view class="sort-box-category" v-if="categoryFilter">
        <view :class="'item ' + (item.checked ? 'active' : '')" :data-category-index="index" :key="id" @tap="selectCategory" v-for="(item, id) in filterCategory">{{item.name}}</view>
      </view>
    </view>
    <view class="cate-item">
      <view class="b">
        <navigator :class="'item ' + ((iindex + 1) % 2 == 0 ? 'item-b' : '')" :key="iindex" :url="'/pages/goods/goods?id=' + iitem.id" v-for="(iitem, iindex) in goodsList">
