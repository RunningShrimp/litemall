...
<view class="container">
  ...
  <view class="catalog">
    <scroll-view class="nav" scroll-y="true">
      <view :class="'item ' + ( currentCategory.id == item.id ? 'active' : '')" :data-id="item.id" :data-index="index" :key="id" @tap="switchCate" v-for="(item, id) in categoryList">{{item.name}}</view>
    </scroll-view>
    <scroll-view class="cate" scroll-y="true">
      ...
      <view class="bd">
        <navigator :class="'item ' + ((index+1) % 3 == 0 ? 'last' : '')" :key="index" :url="'/pages/category/category?id=' + item.id" v-for="(item, index) in currentSubCategoryList">
