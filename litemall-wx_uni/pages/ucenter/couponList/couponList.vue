  ...
  <view class="b">
    ...
    <view class="coupon-form" v-if="status == 0">
      <view class="input-box">
        ...
        <van-icon @tap.stop="clearExchange" class="clear-icon" name="close" v-if="code.length > 0"></van-icon>
      </view>
      <view @tap="goExchange" class="add-btn">兑换</view>
    </view>

    <view class="help" v-if="status == 0">
      <van-icon name="question-o"></van-icon>使用说明</view>

    <scroll-view :scroll-top="scrollTop" class="coupon-list" scroll-y="true">

      <view :class="'item ' + ( status == 0 ? 'active' : '')" :key="index" v-for="(item, index) in couponList">
