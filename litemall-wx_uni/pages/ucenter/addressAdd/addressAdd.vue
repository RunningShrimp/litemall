...
<view class="add-address">
  <view class="add-form">
    <view class="form-item">
      <input :value="address.name" @input="bindinputName" auto-focus class="input" placeholder="姓名"></input>
    </view>
    <view class="form-item">
      <input :value="address.tel" @input="bindinputMobile" class="input" placeholder="手机号码"></input>
    </view>
    <view class="form-item">
      <input :value="address.province + address.city + address.county" @tap="chooseRegion" class="input" disabled="true" placeholder="省份、城市、区县"></input>
    </view>
    <view class="form-item">
      <input :value="address.addressDetail" @input="bindinputAddress" class="input" placeholder="详细地址, 如街道、楼盘号等"></input>
    </view>
  ...
  <view class="btns">
    <button @tap="cancelAddress" class="cannel">取消</button>
    <button @tap="saveAddress" class="save">保存</button>
  </view>

  <view class="region-select" v-if="openSelectRegion">
    <view class="hd">
      <view class="region-selected">
        <view :class="'item ' + (item.code == 0 ? 'disabled' : '') + ' ' + ((regionType -1) === index ? 'selected' : '')" :data-region-type-index="index" :key="code" @tap="selectRegionType" v-for="(item, code) in selectRegionList">{{item.name}}</view>
      </view>
      <view :class="'done ' + (selectRegionDone ? '' : 'disabled')" @tap="doneSelectRegion">确定</view>
    </view>
    <view class="bd">
      <scroll-view class="region-list" scroll-y>
        <view :class="'item ' + (item.selected ? 'selected' : '')" :data-region-index="index" :key="code" @tap="selectRegion" v-for="(item, code) in regionList">{{item.name}}</view>
      </scroll-view>
    </view>
  </view>
</view>
<view @tap="cancelSelectRegion" class="bg-mask" v-if="openSelectRegion"></view>
</view>
</template>
...
global['__wxRoute'] = 'pages/ucenter/addressAdd/addressAdd';
...
export default global['__wxComponents']['pages/ucenter/addressAdd/addressAdd'];
</script>
<style>
