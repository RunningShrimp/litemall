...
<view class="container">
  ...
  <van-cell-group title="操作">
    <van-field :value="aftersale.typeDesc" @tap="showTypePicker" clickable input-align="right" label="退款类型" placeholder="请选择" required></van-field>
    <van-field :value="aftersale.reason" @change="onReasonChange" clearable input-align="right" label="退款原因" placeholder="请输入" required></van-field>
    <van-cell :value="'￥' + aftersale.amount + '元'" label="不可修改，即订单实付- 运费" required title="退款金额"></van-cell>
    <van-field :value="aftersale.comment" autosize input-align="right" label="退款说明" placeholder="请输入" type="textarea"></van-field>
    <van-cell label="最多上传三张" title="上传凭证">
      <van-uploader :file-list="fileList" @after-read="afterRead" @delete="deleteImage" max-count="3" preview-size="50px"></van-uploader>
    </van-cell>    
  </van-cell-group>
