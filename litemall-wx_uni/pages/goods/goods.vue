...
<view class="container">
  <swiper autoplay="true" class="goodsimgs" duration="1000" indicator-dots="true" interval="3000">
    <swiper-item :key="index" v-for="(item, index) in goods.gallery">
  ...
  <view class="goods_name">
    ...
    <view :hidden="!canShare" @tap="shareFriendOrCircle" class="goods_name_right">分享</view>
  </view>
  <view :hidden="!openShare" class="share-pop-box">
    <view class="share-pop">
      <view @tap="closeShare" class="close">
      ...
      <view class="share-info">
        ...
        <button @opensetting="handleSetting" class="savesharebtn" open-type="openSetting" v-if="(!isGroupon) && (!canWrite)">
          <image class="sharebtn_image" src="/static/static/images/friend.png"></image>
          <view class="sharebtn_text">发朋友圈</view>
        </button>
        <button @tap="saveShare" class="savesharebtn" v-if="!isGroupon && canWrite">
          <image class="sharebtn_image" src="/static/static/images/friend.png"></image>
          <view class="sharebtn_text">发朋友圈</view>
        </button>
      </view>
  ...
  <view @tap="switchAttrPop" class="section-nav section-attr">
  ...
  <view class="comments" v-if="comment.count > 0">
    <view class="h">
      <navigator :url="'/pages/comment/comment?valueId=' + goods.id + '&type=0'">
        ...
        <view class="i">查看全部<van-icon name="arrow"></van-icon>
        </view>
      </navigator>
    </view>
    <view class="b">
      <view :key="id" class="item" v-for="(item, id) in comment.data">
        <view class="info">
          <view class="user">
        ...
        <view class="imgs" v-if="item.picList.length > 0">
          <image :key="index" :src="iitem" class="img" v-for="(iitem, index) in item.picList"></image>
        </view>
  ...
  <view class="goods-attr">
    ...
    <view class="l">
      <view :key="name" class="item" v-for="(item, name) in attribute">
  ...
  <view class="detail">
    ...
    <view v-html="article_goodsDetail"></view>
  </view>
  ...
  <view class="related-goods" v-if="relatedGoods.length > 0">
    ...
    <view class="b">
      <view :key="id" class="item" v-for="(item, id) in relatedGoods">
...
<!-- 规格选择界面 -->
<view :hidden="!openAttr" class="attr-pop-box">
  <view class="attr-pop">
    <view @tap="closeAttr" class="close">
      <van-icon class="icon" name="cross"></van-icon>      
    </view>
    <view class="img-info">
      <image :src="goods.picUrl" class="img"></image>
      <view class="info">
        <view class="c">
    ...
    <!-- 规格列表 -->
    <view class="spec-con">
      <view :key="name" class="spec-item" v-for="(item, name) in specificationList">
        <view class="name">{{item.name}}</view>
        <view class="values">
          <view :class="'value ' + (vitem.checked ? 'selected' : '')" :data-name="vitem.specification" :data-value-id="vitem.id" :key="id" @tap="clickSkuValue" v-for="(vitem, id) in item.valueList">{{vitem.value}}</view>
        </view>
      </view>
      ...
      <view class="spec-item" v-if="groupon.length > 0">
        ...
        <view class="values">
          <view :class="'value ' + (vitem.checked ? 'selected' : '')" :data-name="vitem.specification" :data-value-id="vitem.id" :key="index" @tap="clickGroupon" v-for="(vitem, index) in groupon">￥{{vitem.discount}} ({{vitem.discountMember}}人)</view>
        </view>
      </view>
      ...
      <view class="number-item">
        ...
        <view class="selnum">
          <view @tap="cutNumber" class="cut">-</view>
          <input :value="number" class="number" disabled="true" type="number"></input>
          <view @tap="addNumber" class="add">+</view>
        </view>
      </view>
...
<view class="bottom-btn">
  <view @tap="addCollectOrNot" class="l l-collect" v-if="!isGroupon">
    <van-icon class="icon" color="#ab956d" name="star" v-if="collect"></van-icon>
    <van-icon class="icon" name="star-o" v-else></van-icon>
  </view>
  <view class="l l-cart" v-if="!isGroupon">
    <view class="box">
      <text class="cart-count">{{cartGoodsCount}}</text>
      <van-icon @tap="openCartPage" class="icon" name="cart-o"></van-icon>
    </view>
  </view>
  <view @tap="addToCart" class="r" v-if="!soldout">加入购物车</view>
  <view @tap="addFast" class="c" v-if="!soldout">{{isGroupon?'参加团购':'立即购买'}}</view>
...
global['__wxRoute'] = 'pages/goods/goods';
var app = getApp();
...
export default global['__wxComponents']['pages/goods/goods'];
</script>
<style>
@import "./goods.css";
</style>