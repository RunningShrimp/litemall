...
<template>
	<view class="interlayer">
		<block v-bind:key="index" v-for="(item, index) in nodes">
			...
			<rich-text :nodes="[item]" class="_entity" v-else-if="item.decode"></rich-text>
			<!--#endif-->
			<text decode v-else-if="item.type=='text'">{{item.text}}</text>
			...
			<!--#endif-->
			<!--列表-->
			<view :class="item.attrs.class" :style="(item.attrs.style||'')+';display:flex'" v-else-if="item.name=='li'">
