...
<view :style="'height: ' + innerHeight" class="van-tree-select">
  <scroll-view class="van-tree-select__nav" scroll-y>
    <van-sidebar :active-key="mainActiveIndex" @change="onClickNav" custom-class="van-tree-select__nav__inner">
      <van-sidebar-item :disabled="item.disabled" :key="index" :title="item.text" active-class="main-active-class" custom-class="main-item-class" disabled-class="main-disabled-class" v-for="(item, index) in items"></van-sidebar-item>
    </van-sidebar>
  </scroll-view>
  <scroll-view class="van-tree-select__content" scroll-y>
    <slot name="content"></slot>
    <view :class="'van-ellipsis content-item-class ' + ( utils.bem('tree-select__item', { active: wxs.isActive(activeId, item.id), disabled: item.disabled }) ) + ' ' + ( wxs.isActive(activeId, item.id) ? 'content-active-class' : '' ) + ' ' + ( item.disabled ? 'content-disabled-class' : '' )" :data-item="item" :key="id" @tap="onSelectItem" v-for="(item, id) in subItems">{{ item.text }}<van-icon class="van-tree-select__selected" name="checked" size="16px" v-if="wxs.isActive(activeId, item.id)"></van-icon>
    </view>
  </scroll-view>
</view>
</template>
