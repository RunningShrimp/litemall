<template>
<van-cell :arrow-direction="arrowDirection" :border="border" :center="center" :clickable="clickable" :custom-style="customStyle" :icon="leftIcon" :is-link="isLink" :required="required" :size="size" :title="label" :title-width="titleWidth" custom-class="van-field">
  ...
  <view :class="utils.bem('field__body', [type, system])">
    ...
    <van-icon @touchstart="onClear" class="van-field__clear-root van-field__icon-root" name="clear" size="16px" v-if="clearable && focused && value && !readonly"></van-icon>
  ...
  <view :class="'van-field__error-message ' + utils.bem('field__error', [errorMessageAlign, { disabled, error }])" v-if="errorMessage">{{ errorMessage }}</view>
</van-cell>
</template>
