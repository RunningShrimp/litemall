<template>
    <view class="van-index-bar">
        ...
        <view @tap.stop="onClick" catch:touchcancel="onTouchStop" catch:touchend="onTouchStop"
              catch:touchmove="onTouchMove" class="van-index-bar__sidebar" v-if="showSidebar">
            <view :data-index="index" :key="index"
                  :style="'z-index: ' + ( zIndex + 1 ) + '; color: ' + ( activeAnchorIndex === index ? highlightColor : '' )"
                  class="van-index-bar__index" v-for="(item, index) in indexList">{{ item }}
            </view>
        </view>
    </view>
</template>
