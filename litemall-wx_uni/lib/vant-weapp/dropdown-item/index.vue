<template>
    <view :class="utils.bem('dropdown-item', direction)" :style="wrapperStyle" v-if="showWrapper">
        <van-popup :close-on-click-overlay="closeOnClickOverlay" :duration="transition ? duration : 0"
                   :overlay="overlay" :position="direction === 'down' ? 'top' : 'bottom'" :show="showPopup"
                   @close="onClickOverlay" custom-style="position: absolute" overlay-style="position: absolute">
            <van-cell :class="utils.bem('dropdown-item__option', { active: item.value === value } )" :data-option="item"
                      :icon="item.icon" :key="index" @tap="onOptionTap" clickable v-for="(item, index) in options">
                <view :style="item.value === value  ? 'color:' + activeColor : ''" class="van-dropdown-item__title"
                      slot="title">{{ item.text }}
                </view>
                <van-icon :color="activeColor" class="van-dropdown-item__icon" name="success"
                          v-if="item.value === value"></van-icon>
            </van-cell>
