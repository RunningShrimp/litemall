...
<view :class="'custom-class ' + utils.bem('tabs', [type])">
  <van-sticky :container="container" :disabled="!sticky" :offset-top="offsetTop" :z-index="zIndex" @scroll="onTouchScroll">
    <view :class="utils.bem('tabs__wrap', { scrollable }) + ' ' + ( type === 'line' && border ? 'van-hairline--top-bottom' : '' )">
      <slot name="nav-left"></slot>

      <scroll-view :class="utils.bem('tabs__scroll', [type])" :scroll-left="scrollLeft" :scroll-x="scrollable" :style="color ? 'border-color: ' + color : ''" scroll-with-animation>
        <view :class="utils.bem('tabs__nav', [type]) + ' nav-class'">
          <view :style="lineStyle" class="van-tabs__line" v-if="type === 'line'"></view>
          <view :class="getters.tabClass(index === currentIndex, ellipsis) + ' ' + utils.bem('tab', { active: index === currentIndex, disabled: item.disabled, complete: !ellipsis })" :data-index="index" :key="index" :style="getters.tabStyle(index === currentIndex, ellipsis, color, type, item.disabled, titleActiveColor, titleInactiveColor, swipeThreshold, scrollable)" @tap="onTap" v-for="(item, index) in tabs">
            <view :class="ellipsis ? 'van-ellipsis' : ''" :style="item.titleStyle">{{ item.title }}<van-info :dot="item.dot" :info="item.info" custom-class="van-tab__title__info" v-if="item.info !== null || item.dot"></van-info>
            </view>
          </view>
        </view>
      </scroll-view>
...
global['__wxRoute'] = 'lib/vant-weapp/tabs/index';
...
export default global['__wxComponents']['lib/vant-weapp/tabs/index'];
</script>
<style>
@import "./index.css";
</style>